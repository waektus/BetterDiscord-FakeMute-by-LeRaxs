...
module.exports = class FakeMuteByLeRaxs {
    ...
    start() {
        ...
        this.registerKeyboardShortcut();
        console.log('แอบฟังอยู่นะจ้ะ by LeRaxs: เริ่มทำงานแล้ว v1.2.0');
    }
    ...
    /**
     * หา MediaEngineActions จาก webpack — รองรับทั้ง key เก่าและใหม่ของ Discord
     */
    _getMediaEngineActions() {
        const candidates = [
            ['toggleSelfMute', 'toggleSelfDeaf'],
            ['setLocalMute', 'toggleSelfDeaf'],
            ['toggleSelfMute', 'setLocalDeafen'],
        ];
    ...
    /**
     * อ่านสถานะ mute/deaf ปัจจุบัน
     */
    _getCurrentAudioState() {
        try {
            const store = BdApi.Webpack.getStore('MediaEngineStore');
            if (store) {
                return {
                    muted: store.isSelfMute?.() ?? false,
                    deafened: store.isSelfDeaf?.() ?? false,
                };
            }
        } catch (e) { }
        const vs = this.getVoiceState();
        return {
    ...
    /**
     * เมื่อเปิด FakeMute → บันทึกสถานะ mute/deaf ปัจจุบันไว้
     * ไม่ต้องทำอะไรกับ mute/deaf ณ จุดนี้
     */
    _saveStateOnEnable() {
        this._stateBeforeFakeMute = this._getCurrentAudioState();
    }

    /**
     * เมื่อปิด FakeMute (WS unblock แล้ว) → จัดการ mute/deaf ตาม logic:
     *
     *  ถ้าก่อนเปิด FakeMute:  mute=true,  deaf=true   → ปิดหูฟัง(deaf→false) แต่ไมค์ยังปิด
     *  ถ้าก่อนเปิด FakeMute:  mute=true,  deaf=false  → ไม่ต้องทำอะไร (เหมือนเดิม)
     *  ถ้าก่อนเปิด FakeMute:  mute=false, deaf=false  → เปิดหูฟัง แต่ไมค์ยังปิด
     *                                                     (เพราะ FakeMute ปิดไมค์ไปก่อนหน้า)
     *
     *  สรุป: WS unblock แล้ว → ตรวจสถานะ deaf ปัจจุบัน → ถ้า deaf อยู่ → toggle deaf (เปิดหูฟัง)
     *                                                      → ไมค์ไม่แตะ (คงสถานะปิดไมค์เดิม)
     */
    _applyOnDisable() {
        if (!this.settings.autoDeafenOnEnable) return;

        const actions = this._getMediaEngineActions();
        if (!actions) {
            console.warn('แอบฟังอยู่นะจ้ะ: หา MediaEngineActions ไม่เจอ, ข้าม autoDeafen');
            return;
        }

        const toggleDeaf = actions.toggleSelfDeaf ?? actions.setLocalDeafen;
        if (!toggleDeaf) return;

        // WS unblock แล้ว → อ่านสถานะจริง
        const state = this._getCurrentAudioState();

        // ถ้าตอนนี้ deaf อยู่ → เปิดหูฟัง (packet ออกจริงเพราะ WS unblock แล้ว)
        // ไมค์ไม่แตะ → ยังปิดอยู่
        if (state.deafened) {
            toggleDeaf(); // deaf: true → false  (packet ออกจริง ✓)
        }
    }
    ...
    toggleFixate(status = null) {
        ...
        if (enabling) {
            // ── เปิด FakeMute ──────────────────────────────────────────────
            // บันทึกสถานะเดิมก่อนบล็อก
            this._saveStateOnEnable();
            ...
            // ขั้น 1: unblock WS ก่อน
            this.fixated = false;
            this.disableFakeMute();

            // ขั้น 2: หลัง unblock → จัดการ deaf/mute
            // ใช้ setTimeout เล็กน้อยให้ WS unblock สมบูรณ์ก่อน Actions ส่ง packet
            setTimeout(() => this._applyOnDisable(), 100);
        }
